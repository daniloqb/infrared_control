...
void IRsend::sendYork2(unsigned char IRcode[], int nbits)
{

  enableIROut(38);
  mark(YORK_HDR_MARK);
  space(YORK_HDR_SPACE);

for (int j = 0; j<17; j++){
  
   for (int i = 0; i < nbits; i++) {
    if (IRcode[j] & YORK_BOTBIT) {
      mark(YORK_BIT_MARK);
      space(YORK_ONE_SPACE);
    } 
    else {
      mark(YORK_BIT_MARK);
      space(YORK_ZERO_SPACE);
    }
    IRcode[j] >>= 1;
  }//end for i 
...
//YORK send 272 bits so I divided de data into 4 parts.

//void IRsend::sendYork(unsigned long data0, unsigned long data1, unsigned long data2 ,unsigned long data3, unsigned long data4, int nbits)





void IRsend::sendYork(unsigned long IRcode[], int nbits)
{
  enableIROut(38);
  mark(YORK_HDR_MARK);
  space(YORK_HDR_SPACE);
  ...
  for (int i = 0; i < YORK_LASTBITS; i++) {
    if (IRcode[4] & YORK_TOPBIT) {
      mark(YORK_BIT_MARK);
      space(YORK_ONE_SPACE);
    } 
    else {
